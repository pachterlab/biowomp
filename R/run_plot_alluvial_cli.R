...
run_plot_alluvial_cli <- function(args) {
    if (length(args) == 0 || any(args %in% c("--help", "-h"))) {
        cat("
Usage: biowomp plot_alluvial --input INPUT [options]
...
Optional:
  ...
  --disable_keep_x_labels Keep x labels
...
")
    ...
    keep_x_labels <- store_false(args, c("--disable_keep_x_labels"))
